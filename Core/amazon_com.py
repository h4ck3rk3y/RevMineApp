...
amazon_link = "http://www.amazon.com/product-reviews/%s?sortBy=helpful&pageNumber=%d"
...
def alternates(pid, li):
	...
	url = "http://www.amazon.com/dp/" + pid
	print url
	soup = 	make_soup(url)
	...
	try:
		price = float(span[0].text.strip().replace(",","")[1:])
	except:
		price = 99999

	price_range = get_price_range(price, "USA")
	all_products_url = "http://amazon.com" + next_url + '&low-price=' + str(price_range[0]) + '&high-price=' + str(price_range[1])
